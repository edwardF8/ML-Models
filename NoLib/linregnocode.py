...
with open('NoLib\medical_cost.csv','r') as file: #opens csv and saves under file
    ...
    for row in csv_reader:
        data.append(row)
for row in data:
    row[3] = float(row[3]) / 53.1 #scaling BMI to values in between 0 and 1
    row[7] = float(row[5]) / 63770.42801 #scaling price, will multiply label by 63770.42801
    row[4] = float(row[5]) /5.0
    if(row[2] == "female"):
        row[2] = float(1)
    else:
        row[2] = float(0)
